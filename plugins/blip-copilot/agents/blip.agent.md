...
**BLOCKER**: start a new `blip-implement` task with the fix described, then re-review. If the same blocker appears twice, discard the worktree (`git worktree remove --force .blip/worktrees/<task_id> && git branch -D blip/<task_id>`) and report.
