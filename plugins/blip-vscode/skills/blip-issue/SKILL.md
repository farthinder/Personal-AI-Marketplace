...
description: >
  ...
  description of desired functionality or a problem that needs solving.
---
...
1. **`.github/` directory** — its presence strongly implies GitHub Issues
2. **Project instruction files** — look for mentions of GitHub, Linear, Jira, project keys,
   or tracker URLs in `.github/copilot-instructions.md`, `AGENTS.md`, etc.
...
Use the GitHub CLI via terminal:

```bash
gh issue create --title "<title>" --body "<body>"
```

If `gh` is not available or not authenticated, output the formatted Markdown so the user
can paste it themselves, and say so clearly.

After creating, share the issue URL so the user can find it immediately.
